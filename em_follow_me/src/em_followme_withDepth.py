...
# Paper     : Human Tracking and Following of Mobile Robot with a Laser Scanner
# Author    : Qinghua Xiao, Fuchun Sun, Rui Ge, Kunlun Chen, Bin Wang
# Conference: 2017 2nd International Conference on Advanced Robotics and Mechatronics(ICARM)
...
class GetURG(object):
    ...
    def tracePosition(self, position, urg_deg, size):
        ...
        if position[1] > 0.8:
            vel.linear.x = min(self.pre_vel.linear.x + 0.02, position[1] * 0.1 + 0.2, MAX_SPEED)
        elif position[1] < 0.5:
            vel.linear.x = -0.1
        else :
    ...
    def __init__(self):
        ...
        for x in xrange(DATA_SIZE):
            hoge = np.loadtxt("dataset/output" + str(x+1) + ".csv", delimiter=",")
