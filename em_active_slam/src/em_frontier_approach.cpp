...
#define X_SIZE 2048 //X軸セル数
#define Y_SIZE 2048 //Y軸セル数
...
//                if(x_close < 1004 && x_close > 1044 && y_close < 1004 && y_close > 1044) {
        ...
        if(cell_x.size() != 0){
            ...
            for(int i=0; i<cell_x.size(); i++){
                frontier_pose_x.push_back((cell_x[i] - 1024) * 0.05);
                frontier_pose_y.push_back((cell_y[i] - 1024) * 0.05);
...
int main(int argc, char **argv)
{
    ...
    ros::Subscriber status_sub = nh.subscribe<actionlib_msgs::GoalStatusArray>("/move_base/move/status", 1, StatusCallback);
